...
export function CbtGlobalPublishingPanel({
  ...
  const [aiFeedback, setAiFeedback] = useState<{ message: string; error: boolean } | null>(null);
  ...
  async function generateAiContent() {
    ...
    if (!categoryId || !categoryLeafVerified) {
      setAiFeedback({ error: true, message: "请先在第 1 步选择并确认最底层 CBT 分类，再使用 AI 生成。" });
      return;
    }
    setBusy("ai-content");
    setStatus("");
    ...
    try {
      ...
      if (!globalTitle.trim()) reconstructionFields.push("title");
      if (!description.trim()) reconstructionFields.push("description");
      // One backend operation runs both provider calls concurrently, retries
      // only the failed field, and persists nothing until both are valid.
      ...
      setExecution(null);
      onReviewInvalidated();
      setAiFeedback({ error: false, message: "AI 英文标题和描述已全部生成，请检查后保存。" });
    } catch (error) {
      if (activeDraftIdRef.current !== targetDraftId) return;
      ...
      setAiFeedback({ error: true, message });
    } finally {
  ...
  return <section className="workspace wf-listing-editor">
    ...
    <div className="wf-listing-layout">
      ...
      <section id="description" className="surface wf-section"><div className="wf-section-title"><span>4</span><div><h3>描述</h3><p>英文、基于采集信息；不出现品牌。标题旁的 AI 按钮会与标题并发生成，全部完成后一次性回填。</p></div></div><label>英文商品描述 *<textarea disabled={busy === "ai-content"} rows={10} value={description} onChange={(event) => { setDescription(sanitizeCbtDescription(event.target.value)); setSaved(null); setPreview(null); }} /></label></section>
