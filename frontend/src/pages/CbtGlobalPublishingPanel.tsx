...
export function CbtGlobalPublishingPanel({
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([listStores(), getCbtListingConfig(draftId)])
      .then(([storeRows, config]) => {
        if (cancelled) return;
        setStores(storeRows);
  ...
  async function saveConfig() {
    if (!canSave) return;
    setBusy("save"); setStatus(""); setPreview(null);
    try {
  ...
  return <section className="workspace cbt-workspace">
    ...
    <section className="surface publish-section">
      ...
      {preview && <div className={`validation-result ${preview.allowed ? "ready" : "blocked"}`}><strong>{preview.allowed ? "官方 Global Selling 请求已通过结构校验" : "刊登请求未通过"}</strong>{preview.errors.map((error) => <span key={error}>{error}</span>)}{preview.allowed && <span>下一步将接入人工审批和受控发布队列；本操作没有向美客多创建商品。</span>}</div>}
