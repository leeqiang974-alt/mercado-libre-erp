...
function formatSourcePrice(price: number, currency?: string): string {
...
export function CbtGlobalPublishingPanel({
  ...
  const [aiFeedback, setAiFeedback] = useState<{ target: "title" | "description" | "both"; message: string; error: boolean } | null>(null);
  ...
  const offersValid = offers.every((offer) => {
    const title = normalizeCbtTitle(offer.title);
  ...
  const canSave = Boolean(
    ...
    && offers.length > 0 && offersValid && missing.length === 0,
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([storeRows, config, system, savedPricing, persistedDraft]) => {
        if (cancelled) return;
        if (persistedDraft.source_product_id) {
          getSourceProduct(persistedDraft.source_product_id)
            .then((src) => {
              if (cancelled) return;
              const fullTitle = String(src.snapshot?.title ?? "").trim();
              if (fullTitle) {
                setSourceTitle(fullTitle);
                // 分类搜索框显示完整原标题，便于人工参考与关键词匹配
                setCategorySearchQuery(normalizeCbtTitle(fullTitle));
              }
            })
            .catch(() => { /* 源商品不可用时静默，保留草稿标题 */ });
        }
        setStores(storeRows);
        setReadiness(system);
  ...
  async function recollectListingDraft(event: { stopPropagation: () => void }, item: ProductDraftRead) {
    ...
    try {
      ...
      if (item.source_variant_asin) {
        // 变体草稿必须采变体页：父链接与变体可能规格/件数不同（如 2pcs vs 10pcs），
        ...
        const variantResult = await new Promise<{ ok: boolean; error?: string }>((resolve) => {
          ...
          timer = window.setTimeout(() => {
            finishVariant({ ok: false, error: "本机 Amazon 插件未返回结果，请确认插件已启用。" });
          }, 30000);
          function variantHandler(event: Event) {
            const detail = (event as CustomEvent<{ sourceProductId?: number; ok?: boolean; error?: string }>).detail;
            if (detail?.sourceProductId !== job.source_product_id) return;
            finishVariant({ ok: detail.ok === true, error: detail.error });
          }
          window.addEventListener("meli-amazon-recollect-result", variantHandler);
          window.dispatchEvent(new CustomEvent("meli-amazon-recollect", {
            detail: { sourceProductId: job.source_product_id, sourceUrl: job.source_url },
          }));
  ...
  function updateGlobalTitle(value: string) {
    const nextTitle = normalizeCbtTitle(value);
    setOffers((current) => current.map((offer) => {
      const currentTitle = normalizeCbtTitle(offer.title);
      // Keep a valid site-specific title that the operator deliberately
      // customized. Empty, previously mirrored, or invalid overlong titles
      // continue following the global title automatically.
      if (currentTitle && currentTitle !== globalTitle && currentTitle.length <= 60) return offer;
      return { ...offer, title: nextTitle };
    }));
    setGlobalTitle(nextTitle);
    setSaved(null);
    setPreview(null);
    setExecution(null);
  }

  async function autoFixListingCategory(
    event: { stopPropagation: () => void },
    item: { id: number; title?: string | null },
  ) {
    event.stopPropagation();
    if (!storeId || autoFixBusy !== null) return;
    setAutoFixBusy(item.id);
    try {
      const result = await autoFixDraftCategory(item.id, Number(storeId));
      const updated = await getDraft(item.id);
      setListingRail((current) =>
        current.some((row) => row.id === updated.id)
          ? current.map((row) => (row.id === updated.id ? updated : row))
          : [updated, ...current],
      );
      onDraftChange?.(updated);
      setStatus(`已按标题建议自动校正分类为 ${result.category_id}${result.changed ? "" : "（未变化）"}；属性已清空，需重新加载属性后再次发布。`);
    } catch (error) {
      setStatus(`分类校正失败：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setAutoFixBusy(null);
    }
  }
  ...
  async function generateAiContent(requestedFields: Array<"title" | "description"> = ["title", "description"]) {
    if (busy.startsWith("ai-")) return;
    setAiFeedback(null);
    const feedbackTarget = requestedFields.length === 1 ? requestedFields[0] : "both";
    const operation = requestedFields.length === 1 ? `ai-${requestedFields[0]}` : "ai-content";
    setBusy(operation);
    setStatus("");
    const targetDraftId = draftId;
    try {
      ...
      const updatedFields = new Set(result.validation.updated_fields ?? ["title", "description"]);
      if (updatedFields.has("title")) updateGlobalTitle(result.title);
      if (updatedFields.has("description")) setDescription(sanitizeCbtDescription(result.description));
      setSaved(null);
      setPreview(null);
      setExecution(null);
      onReviewInvalidated();
      setAiFeedback({
        ...
        error: false,
        message: feedbackTarget === "description"
          ? "AI 英文商品描述已生成，请检查后保存。"
          : "AI 英文标题和描述已全部生成，请检查后保存。",
      });
    } catch (error) {
      if (activeDraftIdRef.current !== targetDraftId) return;
      ...
      if (activeDraftIdRef.current === targetDraftId) setBusy("");
    }
  }
  ...
  async function setCoverImage(url: string) {
    ...
    try {
      setBusy("media");
      const updated = await saveCurrentDraftContent({ title: normalizeCbtTitle(globalTitle || draft.title || ""), description: sanitizeCbtDescription(description.trim() || draft.description || ""), brand: "Unbranded", image_urls, video_urls: videoUrls });
      onDraftChange(updated);
      setStatus("主图已保存。");
  ...
  async function removeVideo(url: string) {
    ...
    try {
      setBusy("media");
      const updated = await saveCurrentDraftContent({
        title: normalizeCbtTitle(globalTitle || draft.title || ""),
        description: sanitizeCbtDescription(description.trim() || draft.description || ""),
        brand: "Unbranded",
  ...
  function marketplaceValidationErrors() {
    ...
    for (const offer of offers) {
      const titleLength = normalizeCbtTitle(offer.title).length;
      if (titleLength < 1 || titleLength > 60) {
        errors.push(`${MARKET_NAMES[offer.site_id] ?? offer.site_id}站点标题必须为 1–60 个字符（当前 ${titleLength}）`);
      }
    }
  ...
  return <section className="workspace wf-listing-editor">
    ...
    <div className="wf-listing-layout">
      ...
      <section id="description" className="surface wf-section"><div className="wf-section-title"><span>4</span><div><h3>描述</h3><p>英文、基于采集信息且不出现品牌；手动 AI 不要求先选分类，分类仍需在发布前确认。</p></div><button type="button" className="tiny-button wf-ai-button" disabled={busy.startsWith("ai-")} onClick={() => void generateAiContent(["description"])}><Sparkles size={13} />{busy === "ai-description" ? "描述生成中…" : "AI 生成描述"}</button></div><label>英文商品描述 *<textarea disabled={busy === "ai-content" || busy === "ai-description"} rows={10} value={description} onChange={(event) => { setDescription(sanitizeCbtDescription(event.target.value)); setSaved(null); setPreview(null); }} /></label>{aiFeedback?.target === "description" && <p className={`wf-ai-feedback ${aiFeedback.error ? "error" : "success"}`} role="status">{aiFeedback.message}</p>}</section>
