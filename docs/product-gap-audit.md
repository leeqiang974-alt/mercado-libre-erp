...
- Publish outcomes that may have created an item but cannot be confirmed are quarantined and cannot be retried automatically. Each new job carries a unique seller SKU; the operator can search the authorized seller by that exact reference, while zero/multiple matches stay blocked and a single match must pass frozen seller/site/category/Classic-Premium/non-FULL/description checks before it is accepted.
...
- Combined Claude/NVIDIA audits can be queued for up to 50 independently ready drafts after explicit provider-cost acknowledgement. Active work is deduplicated, provider throttling pauses untouched jobs, and no batch action records approval or publishes a listing.
- Operators can edit saved listing title, description, brand, and image URLs. The edit API preserves Amazon price evidence and controlled site/category/stock/commercial/logistics fields, rejects stale browser versions, and makes all prior review and approval evidence ineligible after a real change.
- Operators can select up to 50 drafts for the publish queue. Queueing requires an explicit acknowledgement and reports queued, existing, not-ready, and missing drafts without weakening any per-draft publish gate or duplicating an identical job.
- Operators can run a side-effect-free readiness check on the current batch selection before acknowledging publication. The check and queue path share one evaluator, and stale browser responses cannot validate a later selection.
- Background publish progress is visible without reopening the page: active jobs refresh until they reach a terminal state, while bounded history retains execution timing, item links, errors, and retry eligibility.
- An operator can cancel a mistaken publish job only while it remains pending. The API and worker use row-level exclusion so cancellation cannot interrupt a claimed publication; a successful cancellation is audited and the unchanged draft can be deliberately queued again.
...
- Store authorization uses the selected site's Mercado Libre/Mercado Livre domain across all 18 configured sites. Signed OAuth state binds the expected site and prevents a different-site seller callback from being persisted.
