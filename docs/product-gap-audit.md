...
- Amazon collection now reads high-resolution `colorImages` galleries and binds `colorToAsin` groups without cross-variant image leakage. Discovered variants can be independently queued to obtain their own page evidence, but complete image sets and per-variant dimensions/weight still cannot be guaranteed on every page; selected-page measurements are never copied to sibling variants.
